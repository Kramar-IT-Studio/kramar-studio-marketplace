...
|---|---|---|---|
| [0001](./docs/architecture/decisions/0001-absorb-archforge-into-kramar-studio-marketplace.md) | 2026-05-10 | Accepted (impl pending) | Поглотить плагин `archforge` в `kramar-studio-marketplace`; `archforge-marketplace` → redirect-stub |
...
- **Двухуровневое версионирование.** `marketplace.json` несёт `version: 0.1.0`, и `plugins/product/plugin.json` тоже несёт `version: 0.1.0`. Какова контрактная связь? Должны ли они двигаться синхронно, и если да — как это форсируется без CI? Если нет — какой инкремент чего что означает для пользователя?
- **Когда выделять новую skill.** Конвенция: ровно две skill'ы на плагин (`<role>-conventions`, `<role>-cycle`). Исключение допускается «когда возникает явно отдельное тело знания» (как `c4-diagrams`, `adr-writing` в `archforge`). Где порог? Сегодня размытый — нужен либо ADR с критериями, либо явный отказ от этого исключения.
...
- **Не расходиться с `archforge` shape.** Структура каталогов, две skill'ы, soft-hooks, lifecycle статусов, frontmatter-контракт — копируются из `archforge` дословно. Любое «творческое отличие» — баг плагина, не legitimate variation. Это страхует marketplace от расходящихся диалектов.
