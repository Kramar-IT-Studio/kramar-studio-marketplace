...
Migrate this project's architect artifacts to the currently installed version of the plugin.
...
Run this command after `/plugin marketplace update kramar-studio-marketplace` and `/plugin install architect@kramar-studio-marketplace` have brought your installed plugin to a new version, especially across minor versions (0.2 → 0.3, etc.).

## What it does

1. Reads the **installed plugin version** from `${CLAUDE_PLUGIN_ROOT}/.claude-plugin/plugin.json`.
2. Reads the **project's last upgrade marker** from `docs/architecture/.architect-version` (a single-line file with a version string). If the file is missing, treats the project as last touched by 0.0.0.
...
5. Updates `docs/architecture/.architect-version` to the installed version.
...
- **Refresh integration block in `AGENTS.md`** if present. The 0.3 block includes the `Research` phase in the workflow diagram and adds language-detection support. Run the equivalent of `/architect:remember-compound-integration` and let it replace the block in place. Detect the project's working language; the new block is written in that language.
- **Add `decision-map.md` skeleton if missing**, but only if there are ≥3 open architectural questions in `ARCHITECTURE.md` or ≥2 ADRs already exist. Otherwise leave a note in chat suggesting `/architect:map` as an optional next step.
...
- **Surface to user**: terminology pass for Russian artifacts is now mandatory (was advisory in 0.2). If the project has Russian artifacts, suggest re-running `/architect:review` on key ADRs to apply the terminology pass.
...
```
./docs/architecture/.architect-version
```

If the file doesn't exist, check for the legacy marker `.krait-arch-version` (used in plugin versions 0.4.0-rc1 and rc2 when the plugin was named `krait_arch`). If found, read its contents and rename it to `.architect-version` as part of the migration. Tell the user: "Detected legacy marker `.krait-arch-version` from pre-rename plugin versions. Renamed to `.architect-version`."
...
If `docs/architecture/` itself doesn't exist, the project hasn't been initialized for architect. Tell the user: "This project hasn't been initialized for architect. Run `/architect:init` first."
...
Write the installed version to `docs/architecture/.architect-version`. This is a one-line file with just the version string and a trailing newline.
...
- Any **recommended next steps** the migrations didn't auto-apply (e.g., "we suggest running `/architect:roast` on ADR-0001 since deep-cycle roast is new in 0.3").
...
- **Project not initialized**: route to `/architect:init`.
...
If the marker equals the installed version: "Project is already at architect <version>. Nothing to migrate. Run `/architect:upgrade` again after updating the plugin."
