...
- **Cycle completeness (own)** — доля feature/decision в собственных проектах с полной цепочкой артефактов (HYP→PRD→SPEC→VAL для product; Discover→…→Review для architect). Цель: ≥80%. *Lagging, квартально, измеряется через `/<role>:status`.*
...
Закрытие оставшихся архитектурных вопросов из `ARCHITECTURE.md §6` после ADR-0001: multi-level versioning (A2), формат миграций (B1), quality control без CI (B2), hook execution environment contract (B3), skill-count threshold (C1), cross-role workflow внутри suite (D4 — разморожен ADR-0001 implementation'ом). Cross-marketplace dependency (A1) уже закрыт через absorption.

_Why it serves the approach:_ мета-форма сама — это контракт; пока в ней дыры (нет процедуры миграций, не зафиксирована семантика версий между плагинами suite, не определён hook env contract), все треки A/B стоят на болоте.
