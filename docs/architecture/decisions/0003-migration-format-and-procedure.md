...
3. **Per-step atomicity + terminal reconcile.** Маркер `.<plugin>-version` пишется после
   каждого успешного шага (после прохождения его `Verification`); падение в середине
   оставляет маркер на последнем завершённом шаге. Это уточнение rule 3 ADR-0002 («атомарно
   с последней миграцией») до per-step гранулярности; совпадает с уже декларированным
   поведением `architect`. Миграции **разреженные** — не у каждого version-шага есть файл
   миграции; отсутствие файла на шаг = легитимный no-op. Поэтому выбор миграций идёт по
   `to > marker AND to ≤ installed`, а после последней применённой миграции маркер **сводится
   к `installed`** (хвост version-шагов без миграций). Так маркер после успешного upgrade
   всегда равен `plugin.json.version` (ADR-0002 rule 3), даже когда installed дальше
   последней миграции (текущий случай `architect`: миграции до 0.3.0, installed 1.1.0).
