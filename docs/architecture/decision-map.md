...
|---|---|---|
| **0 (implementation)** | **ADR-0001 implementation** | Reframe основание — repo move `archforge` в `plugins/architect/`, marketplace.json update, redirect-stub в старом репо. Не цикл — операционная работа. Делается отдельной сессией. До её выполнения сегодняшние треки A/B/C из STRATEGY рассогласованы с decision'ом. |
